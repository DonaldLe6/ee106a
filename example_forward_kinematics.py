...
def baxter_forward_kinematics_from_angles(joint_angles):
    ...
    g_st = np.c_[R, qs[0:3,7]]
    bottom = np.array([0, 0, 0, 1])
    g_st_0 = np.vstack((g_st, bottom))
    xis = [] 
    for i in range(7):
        v = -1*np.cross(ws[0:3,i], qs[0:3,i])
        xi = np.vstack((v, ws[0:3,i])).flatten()
        xis.append(xi)
    xis = np.array(xis).T
    return kfs.prod_exp(xis, joint_angles) @ g_st_0
