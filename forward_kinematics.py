...
import rospy
import numpy as np
import example_forward_kinematics as ct
from sensor_msgs.msg import JointState
# Define the callback method which is called whenever this node receives a 
# message on its subscribed topic. The received message is passed as the first
# argument to callback().
def callback(message):
    positions = np.array(message.position[2:9])[[2, 3, 0, 1, 4, 5, 6]]
    #print(positions)
    R = ct.baxter_forward_kinematics_from_angles(positions)
    print("coords", R) 
    yaw=np.arctan2(R[2,1],R[1,1])
    pitch=np.arctan2(-R[3,1],np.sqrt(R[3,2]^2+R[3,3]^2))
    roll=np.arctan2(R[3,2],R[3,3])
