...
def rotation_3d(omega, theta):
    ...
    r = np.identity(3) + (omega_hat / omega_norm) * np.sin(omega_norm * theta) + ((omega_hat @ omega_hat) / (omega_norm ** 2)) * (1 - np.cos(omega_norm * theta))
    return r
...
def hat_3d(xi):
    ...
    w_hat = skew_3d(w)
    zeros = np.zeros((1, 4))
    xi_hat = np.c_[w_hat, v]
    return np.r_[xi_hat, zeros]
...
def homog_3d(xi, theta):
    ...
    I = np.eye(3)
    if norm == 0:
        return np.r_[np.c_[I, v*theta], bottom]
    else:
        exp = rotation_3d(w, theta)
        right = (1/(norm**2)) * (((I-exp) @ (w_hat@v)) + (np.outer(w, w.T)@v)*theta)
        stack = np.c_[exp, right]
        return np.r_[stack, bottom]
