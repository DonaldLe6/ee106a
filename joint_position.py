...
def map_keyboard(side):
    ...
    while not done and not rospy.is_shutdown():
        angles = input()
        joint_angles = angles.split()
        joint_angles = list(map(float, joint_angles))
        # pre_dict = list(zip(joint_names, joint_angles))
        # mapping = {}
        # mapping = Convert(pre_dict, mapping)
        # print(mapping)
        # limb.set_joint_position_speed(0.3)
        for i in range(len(joint_angles)):
                ...
                theta *= np.pi / 180
                joint_name = joints[i]
                print(joint_name, theta)
                print(limb.joint_angle(joint_name))
                for i in range(100000):
                    delta = theta - limb.joint_angle(joint_name)
                    set_j(limb, joint_name, delta)
        #limb.set_joint_positions(mapping)
